  ...
  return (
    <PayPalScriptProvider
      options={{
        ...
        currency: 'USD',
        intent: 'capture',
        // Enable credit card funding for non-PayPal users
        components: 'buttons',
        // Disable credit funding source to use PayPal's card form
        // enableFunding: 'card',
      }}
    >
      <div className="space-y-6">
        ...
        {/* Price display */}
        <div className="text-center py-4 bg-surface-lighter/50 rounded-xl">
          <p className="text-slate-400 text-sm mb-1">Total Amount</p>
          <p className="text-3xl font-display font-bold text-white">
            ${amount} <span className="text-lg text-slate-400">USD</span>
          </p>
        </div>
        ...
        <motion.div
          ...
          className="flex items-center justify-center gap-3 text-slate-500 text-xs"
        >
          <CreditCard className="w-4 h-4" />
          <span>Pay with credit/debit card via PayPal - no account required</span>
        </motion.div>
